...
const createDebugger = (config = defaultConfig) => (store: any) => {
  ...
  return (next: any) => (action: any) => {
    ...
    // }
    console.log("[JX TEST]",next);
    let startTime = Date.now();
    let before = store.getState();

    let result = next(action);
  
    //console.log("[JX TEST]", next, result, action);
    if (currentConnection) {
      ...
      console.log("[JX TEST]", typeof decycledAction);
      let state = {
        id: startTime,
      ...
      if (!blackListed && typeof action !== 'function') {
         // console.log("[JX TEST]", state);
        currentConnection.send('actionDispatched', state);
      }
    }
    //console.log("[JX TEST] result:", result);
    return result;
  };
};
